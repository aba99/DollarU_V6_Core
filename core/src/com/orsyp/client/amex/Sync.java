...
public class Sync {
	...
	{		
	    ...
	    HashMap<String,Mu>ref_mus_map = new HashMap<String,Mu>();
	    ...
	    HashMap<String,Mu>cur_mus_map = new HashMap<String,Mu>();
		...
		for(String connKey:DUAPIConnections.keySet())
		{
			 ...
			 DuApiConnection conn=DUAPIConnections.get(connKey);
				 ...
				 if(conn.getConnName().substring(0,conn.getConnName().indexOf("/")).toUpperCase().equalsIgnoreCase(refnode))
				 {
					...
					ref_mus_map=conn.getMUsHashMap();
					break;
				 }
		}
			 ...
			 if(setRef && !(conn.getConnName().substring(0,conn.getConnName().indexOf("/")).toUpperCase().equalsIgnoreCase(refnode)))
				{
				 ...
				 prtstm.println("------------------------");
				    ...
				    cur_mus_map= conn.getMUsHashMap(); 
					
				    for(String ref_mus: ref_mus_map.keySet())
				    {
				    	if(!cur_mus_map.containsKey(ref_mus))
				    	{
				    		try{
				    		Mu temp = ref_mus_map.get(ref_mus);
				    		temp.setContext(conn.getContext());
				    		temp.create();
			    			prtstm.println("[ MU] "+temp.getName()+" created on "+conn.getConnName());
				    		}
				    		catch (Exception e) {
					...
					for(String ref_uprs:ref_uprocs_map.keySet())
					{
				    	if(cur_uprocs_map.containsKey(ref_uprs))
				    	{
				    		Uproc temp = cur_uprocs_map.get(ref_uprs);
				    		try{
				    		temp.delete();
				    		
			    			prtstm.println("[UPR] "+temp.getName()+" deleted on "+conn.getConnName());
				    		}
				    		catch (Exception e) {
						...
						try{
				    		Uproc temp_uproc = ref_uprocs_map.get(ref_uprs);
